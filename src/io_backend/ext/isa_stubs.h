...
// Op codes
//
// Deprecated in the CERT ISA and intentionally absent below; do not re-add when
// re-syncing this header:
//   YIELD (0x08)   - jobs with poll/mask_poll already yield
//   ADD (0x0f)     - no use case
//   MOV (0x10)     - no use case
//   POLL_32 (0x13) - aiebu maps it to MASK_POLL_32; CERT only sees MASK_POLL_32
//   SLEEP          - debug only, superseded by the dtrace sleep() action
//                    (never defined here)
